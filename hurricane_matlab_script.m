...
%% Correct the Dates (water level data)
...
%% Correct the Dates (Hurricane data)
...
%% Question 1: Are hurricanes recorded in water level?

% Preliminary visualization of water level (not a final figure)
figure (1); clf
title('water level')
...
% calculate water level anomaly using the predicted water level from the
% dataset
...
% plot each hurricane date as a semi-transparent vertical line
hold on
...
%% Question 2: What is the water level anomaly associated with each storm?

hurricane_max = NaN(8,1);
% create an empty array to store the maximum water level from each storm

for i = 1:length(dates_hurricane)
    min_date = dates_hurricane(i) - 0.5;
    % finds the date 12 hours before the closest hurricane measurement
    max_date = dates_hurricane(i) + 0.5;
    % finds the date 12 hours after the closest hurricane measurement
...
% arrays must be flipped since they are listed in reverse chronological
% order
...
%% Question 3: Does strength, intensity, or both combined have more of an impact on water level?

strength_cats = Hurricane_data_for_dates(:,7);
% extract strength category from original dataset
lats_hurricane = Hurricane_data_for_dates(:,2);
lons_hurricane = -1.*Hurricane_data_for_dates(:,3);

gi_lat = 29.2633;
gi_lon = -89.9566;
% hard-coded lat and lon for grand isle tide station

dists = NaN(8,1);
dist_cats = NaN(8,1);
% generate empty arrays for distance and distance category rankings

for i = 1:length(lats_hurricane)
    arclen = distance(gi_lat, gi_lon, lats_hurricane(i), lons_hurricane(i));
    % distance finds the arc distance between the nearest hurricane
    % measurement and the grand isle tide station based on latitude and
    % longitude coordinates
    dist = deg2km(arclen);
    % converts the arc distance in degrees to km
    dists(i) = dist;
    if dist <= 23
        dist_cats(i) = 5;
...
% calculate aggregate intensity score by multiplying
...
% instant water level represents the water level at the instant that the
% hurricane was measured. However, the max water level was what we ended up
% plotting.

figure(4); clf
subplot(3,1,1)
plot(strength_cats, hurricane_max, ".", MarkerSize = 15)
text(1, 3.25, "Barry (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(1.1, 2.95, "Marco (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(1.1, 2.6, "Cristobal (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(2.1, 2.65, "Nate (H1)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(2.1, 2.2, "Francine (H1)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(1.1, 1, "Claudette (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(4.1, 1.5, "Zeta (H3)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(5, 3.25, "Ida (H4)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
% manual label locations for ease of viewing plots
hold on
p1 = polyfit(strength_cats, hurricane_max, 1); 
x1 = linspace(0,6,100);
y1 = polyval(p1, x1);
plot(x1, y1, "--",'LineWidth', 2)
% calculate the slope and plot it as a line
slope1 = p1(1);
text(0.15, 2, sprintf('Slope = %.2f', slope1))
[r1, pval1] = corr(strength_cats, hurricane_max);
r2_1 = r1^2;
text(0.15, 1, sprintf('R^2 = %.2f\np = %.3f', r2_1, pval1))
% calculate the r^2 and p value to test the validity of the relationship
xlim([0,6])
...
% calculate the slope and plot it as a line
slope2 = p2(1);
text(0.15, 1.8, sprintf('Slope = %.2f', slope2))
[r1, pval1] = corr(dist_cats, hurricane_max);
r2_1 = r1^2;
text(0.15, 1, sprintf('R^2 = %.2f\np = %.3f', r2_1, pval1))
% calculate the r^2 and p value to test the validity of the relationship
xlim([0,6])
...
text(1.1, 3.25, "Barry (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(2.1, 2.95, "Marco (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(5.1, 2.75, "Cristobal (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(3.5, 2.1, "Nate (H1)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(1, 2.1, "Francine (H1)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(2.1, 1, "Claudette (TS)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(12.1, 1.5, "Zeta (H3)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
text(19.5, 3.6, "Ida (H4)",'FontSize',8, 'Color', [0, 0.4470, 0.7410])
% manual label locations for ease of viewing plots
hold on
...
% calculate the slope and plot it as a line
slope3 = p3(1);
text(16, 2, sprintf('Slope = %.2f', slope3))
[r1, pval1] = corr(dist_cats, hurricane_max);
r2_1 = r1^2;
text(16, 1, sprintf('R^2 = %.2f\np = %.3f', r2_1, pval1))
% calculate the r^2 and p value to test the validity of the relationship
xlim([0,21])
